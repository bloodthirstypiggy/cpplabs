...
protected:
    ...
    wall::Wall *defend;
    int distance_from_magic;
    int distance_for_magic;
public:
    Cell() {
        coordinate = std::make_pair(0, 0);
        distance_from_magic = -1;
        distance_for_magic = -1;
        defend = nullptr;
    }
    ...
    bool change_distance(int x) {
        if (distance_from_magic == -1) {
            distance_from_magic = x + 1;
            return true;
        } else {
            if ((distance_from_magic > (x + 1))) {
                distance_from_magic = x + 1;
                return true;
            } else {
                return false;
